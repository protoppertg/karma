...
async def init_db():
    ...
    if not dsn.startswith(
        ...
        raise RuntimeError(
            ...
            "with postgresql:// — you likely pasted "
            "the https:// Supabase project URL. "
            "Correct place: Supabase → Connect → "
            "Connection pooling → Session pooler, "
            "then replace [YOUR-PASSWORD].")
...
# TELEGRAM HELPERS
# ============================================================
TG_BASE = f"https://api.telegram.org/bot" \
...
def IK(*rows):
    kb = []
    for r in rows:
        kb.append([{"text": t, "callback_data": d}
                   for t, d in r])
...
async def ai_call(contents, json_mode=True,
                  max_tokens=1200):
    """contents: str or list of parts."""
    for attempt in range(3):
        await _ai_space()
        t0 = time.monotonic()
        try:
            mime = ("application/json"
                    if json_mode else "text/plain")
...
def parse_duration(text):
    ...
    m = re.search(
        r"(\d+)\s*"
        r"(?:m|min|mins|minute|minutes)\b", t)
    if m:
...
def parse_questions(text):
    """Returns (attempted, correct, wrong)."""
    t = (text or "").lower()
    m = re.search(
        r"(\d+)\s*"
        r"(?:questions?|qs|mcqs?|problems?|"
        r"pyqs?)\b", t)
    if not m:
        return None
    att = int(m.group(1))
    cor = wre = None
    mc = re.search(
        r"(\d+)\s*(?:correct|right|correctly)", t)
    mw = re.search(
        r"(\d+)\s*"
...
def detect_energy(text):
    ...
    mid = ("tired", "sleepy", "cant focus",
           "can't focus", "low energy", "drained")
...
def compute_mastery(events):
    ...
    prior=25;
    alpha=min(0.15, 0.08*volume*source_w);
    volume=log10(1+att)/log10(31).
    One event moves mastery <= 15 points.
...
def day_minutes(u, day: date):
    """available = awake − school − coaching
    − commute − meals − 10% buffer"""
    ...
    for sk, ek, dk in pairs:
        days = u.get(dk)
        if days and wd in days and u.get(sk) \
                and u.get(ek):
            bs = datetime.combine(
                day, u[sk], tzinfo=TZ)
            be = datetime.combine(
...
async def subject_mastery_events(user_id,
                                 subject_id):
    rows = await qrows(
        """SELECT questions_attempted a,
                  questions_correct c,
        ...
        sql = (
            """INSERT INTO mastery
               (user_id, subject_id, topic_id,
                mastery, attempts, correct,
                events, last_evidence, algo)
...
async def recompute(user_id, subject_id,
    ...
    if topic_id:
        ...
        await upsert_mastery(
            user_id, subject_id, topic_id, snap)
    if subject_id:
...
async def apply_revision(user_id, subject_id,
                         topic_id, acc, now):
...
async def lazy_tick(u, now):
    ...
    if (u["energy"] != "normal" and es
            ...
            > 12 * 3600):
        await q(
            """UPDATE users SET energy='normal'
               WHERE id=$1::uuid""", u["id"])
        u["energy"] = "normal"
    await q(
        """UPDATE revisions SET status='due'
           WHERE user_id=$1::uuid
...
# AI ROUTER
# ============================================================
ROUTER_SYS = """You convert a student's Telegram
message into ONE JSON action for a study app.
Output ONLY JSON:
{"intent":"...","fields":{...},
 "confidence":"high|medium|low",
 "reply": short reply or null}

Intents and fields (null for unstated values):
- "log_session": {"subject","topic",
  "questions","correct","wrong","minutes",
  "day"} — day is "today" or "yesterday".
  Used when they REPORT study already done.
...
async def subject_pick_card(u, chat, pid,
                            header):
    subs = await get_subjects(u["id"])
    rows = []
    for s in subs[:6]:
...
async def do_test_result(u, f):
    ...
    base = (f"🧪 <b>{esc(test['name'])}</b>: "
            ...
            f"(test weight).")
    try:
        prompt = (f"A student scored "
                  f"{obt}/{tot} ({pct:.0f}%) "
                  f"in '{test['name']}'. Write "
                  f"ONE encouraging, specific "
                  f"sentence (max 25 words) "
                  f"about what to do next.")
        txt = await ai_call(
...
async def do_mistake_add(u, f):
    ...
    warn = ""
    if reps >= 2:
        warn = ("\n⚠️ Repeated mistake — it "
                "now drives your revision "
                "priority.")
    return (f"🧠 {cnt} mistake(s) banked "
            f"({mtype}).{warn}")
    ...
    for r in rows:
        cands.append({
            ...
            "meta": {
                ...
                "test_in_days": min_test}})
    rows = await qrows(
        """SELECT h.id, h.title, h.due_at,
                  h.total_q, h.completed_q,
                  h.est_minutes,
                  s.name subj,
                  h.subject_id sid
           FROM homework h
...
async def start_live_session(u, chat, cand):
    ...
    await send(
        ...
        f"{esc(cand['title'])}\n"
        f"Tap ✅ when done. I'll ask how "
        f"many questions you solved.",
        IK([("⏸ Pause", "ses:pause"),
            ("✅ Finish", "ses:finish")],
           [("❌ Abandon",
             "ses:abandon")]))
...
async def cmd_plan(u, chat):
    ...
    lines = [f"📅 <b>PLAN — "
             ...
             f"</b>",
             f"Capacity: "
             f"<b>{fm(plan['available'])}</b> "
             "(after school/coaching/meals)",
             f"Planned: "
             f"<b>{fm(plan['allocated'])}</b>",
             ""]
    ...
    if plan["missed"]:
        lines.append(
            f"\n<i>Missed {plan['missed']} "
            f"day(s) — kept urgent items, "
            f"spread the rest.</i>")
    if plan["deferred"]:
...
async def cmd_notes(u, chat):
    ...
    if not rows:
        await send(chat,
                   "📎 No notes yet. Send any "
                   "photo — I'll read it and "
                   "can save the text.",
                   MENU_KB)
        return
    kb_rows = []
...
async def cmd_analytics(u, chat):
    ...
    r = await qrow(
        """SELECT COALESCE(
                 ...
                 mins,
                  COALESCE(
                    SUM(questions_attempted),0)
                 q,
                  COALESCE(
                    SUM(questions_correct),0) c
           FROM study_sessions
           WHERE user_id=$1::uuid
           AND status='finished'
           AND created_at >= $2
    ...
    lines = ["📊 <b>LAST 7 DAYS</b>", "",
             ...
             f" {fm(mins)} {trend}",
             f"Questions  "
             f"{bar(min(100, 100 * q / 600))}"
             f" {q}",
             f"Accuracy   {bar(acc)} "
             f"{acc:.0f}%"]
    subs = await qrows(
        """SELECT s.name,
                  SUM(ss.questions_attempted) q,
    ...
    mast = await qrows(
        """SELECT s.name subj, t.name top,
           ...
           WHERE m.user_id=$1::uuid
           AND m.events>0
           ORDER BY m.mastery ASC LIMIT 5""",
        u["id"])
    if mast:
...
async def quiz_label(u):
    """Smart default quiz topic:
    weakest topic with data, else any
    topic, else 'mixed revision'."""
    r = await qrow(
        """SELECT s.name subj, t.name top
           FROM mastery m
           JOIN topics t ON t.id=m.topic_id
           ...
           WHERE m.user_id=$1::uuid
           AND m.events>0
           ORDER BY m.mastery ASC LIMIT 1""",
        u["id"])
    if r:
        return (f"{r['subj']} — {r['top']}",
                r["subj"], r["top"])
    r = await qrow(
        """SELECT t.name top, s.name subj
           FROM topics t
           JOIN subjects s
             ON s.id=t.subject_id
           WHERE t.user_id=$1::uuid
...
async def start_quiz(u, chat, topic_text,
    ...
    prompt = (
        ...
        f'Rules: exactly 4 options; '
        f'"answer" is the 0-based index of '
        f'the correct option; exactly one '
        f'correct; no trick options; '
...
async def send_quiz_q(chat, qid, qs, idx,
                     label):
    q = qs[idx]
    letters = "ABCD"
    body = (f"🧠 <b>QUIZ — {esc(label)}"
            f"</b> ({idx + 1}/{len(qs)})"
            f"\n\n{esc(q['question'])}\n\n")
    ...
    row = []
    for i in range(4):
        row.append((f"{letters[i]}",
                    f"quiz:{qid}:{idx}:{i}"))
    kb = IK(row,
            [("🛑 Stop", f"quiz:{qid}:stop")])
    await send(chat, body, kb)


async def quiz_answer(u, chat, qid, qi,
                      opt):
    row = await qrow(
        """SELECT * FROM quizzes
           WHERE id=$1::uuid
    ...
    if len(answers) >= len(qs):
        await quiz_finish(
            u, chat, qid, qs, answers, row)
    else:
        await send_quiz_q(
            chat, qid, qs, len(answers),
            row["topic_label"])


async def quiz_finish(u, chat, qid, qs,
                      answers, row):
    correct = 0
    ...
    att = len(answers)
    await q(
        """UPDATE quizzes SET status='done'
           WHERE id=$1::uuid""", qid)
    if att:
        await save_evidence(
            u, row["subject_id"],
            row["topic_id"], att, correct, 0,
            "quiz",
            title=f"quiz: "
                  f"{row['topic_label']}")
    letters = "ABCD"
...
async def ob_start(u, chat):
    ...
    await send(
        ...
        "do — not from plans you forget. "
        "A few quick questions, one at a "
        "time. Optional ones you can "
        "/skip.\n\n" + first[1])
...
async def ob_commit(u, chat):
    ...
    if st.get("exam_date"):
        try:
            ...
            pass
    wake = parse_time(st.get("wake")
                      or "06:30")
    if not wake:
        wake = dtime(6, 30)
    sleep = parse_time(st.get("sleep")
                       or "23:00")
    if not sleep:
        sleep = dtime(23, 0)
    ...
    await q(
        """UPDATE users
           ...
           WHERE id=$1::uuid""",
        u["id"], st.get("name"),
        st.get("exam"), exam_d, wake,
        sleep, sch_s, sch_e, sch_d,
        coa_s, coa_e, coa_d)
    await send(
        chat,
        "✅ <b>You're set up.</b>\n\n"
...
# PHOTO / VOICE
# ============================================================
PHOTO_SYS = """You analyze a photo for a
study app. Read ONLY what is clearly
...
async def handle_photo(u, chat, msg):
    ...
    try:
        ...
        LOG.warning("photo dl failed: %s", e)
        await send(chat,
                   "Couldn't download that "
                   "photo — try again.")
        return
    await send(chat, "🔍 Reading image…")
    try:
    ...
    if ptype == "unknown" or conf == "low":
        await send(chat,
                   "I can't read this "
                   "confidently enough to "
                   "save anything. Could you "
                   "type the key parts?",
                   MENU_KB)
        return
    if ptype == "syllabus" \
...
async def handle_voice(u, chat, msg):
    try:
        ...
        LOG.warning("voice dl failed: %s", e)
        await send(chat,
                   "Couldn't download that "
                   "voice note.")
        return
...
async def tutor_reply(u, chat, question):
    ...
    prompt = (
        f"STUDENT CONTEXT:\n{ctx}\n\n"
        f"Answer this study question. "
        f"Be clear and concise (max 200 "
        f"words), one concrete example, "
        f"one common mistake to avoid. "
        f"Never invent statistics about "
        f"the student.\n\n"
        f"QUESTION: {question}")
    key = "tu:" + hashlib.md5(
        prompt.encode()).hexdigest()
...
async def candidate_log_session(
        ...
        minutes, subject=None, topic=None,
        day=None, topic_maybe=None,
        origin="fast", confidence="high"):
    subs = await get_subjects(u["id"])
    if not subs:
        await send(chat,
    ...
    if questions:
        ...
        lines.append(
            "Questions: <b>not given</b>")
    if minutes:
        lines.append(
            f"Duration: <b>{fm(minutes)}"
            f"</b>")
    lines.append(
        f"Subject: <b>"
...
async def candidate_test_add(u, chat, f,
                            conf):
    subs = await get_subjects(u["id"])
    sid, sname = resolve_subject(
        f.get("subject"), subs)
...
async def candidate_mistake(u, chat, f,
                           conf):
    subs = await get_subjects(u["id"])
    sid, sname = resolve_subject(
        f.get("subject"), subs)
    ...
    fields = {
        ...
        "topic": f.get("topic"),
        "count": clampi(
            f.get("count"), 1, 50) or 1,
        "mtype": mtype,
        "description": (f.get("description")
                        or "")[:200]}
    pid = await new_pending(
        u, "mistake_add", fields, "ai")
    if not sid and subs:
        header = (f"🧨 <b>"
                  f"{fields['count']} × "
                  f"{esc(mtype)} "
...
async def session_ctl(u, chat, op):
    ...
    if op == "pause":
        ...
        await send(
            chat,
            f"✅ Finished — {fm(dur)}.\n\n"
            f"How many questions did "
            f"you attempt?\n"
            f"e.g. '25 questions 18 "
            f"correct' (or /skip)",
            IK([("🤷 Skip",
                 f"cfm:{pid}:skipq")]))
    elif op == "abandon":
...
async def handle_callback(cb):
    ...
    try:
        ...
        LOG.exception("callback error: %s",
                      data)
        await send(chat,
                   "Something broke — the "
...
async def go_cb(u, chat, rest):
    ...
    if cand["kind"] == "quiz":
        meta = cand.get("meta", {})
        label = cand.get("chapter") \
            or cand["title"]
        await start_quiz(
            u, chat, meta.get("topic"),
            count=5,
...
async def confirm_cb(u, chat, msg_id,
                     rest):
